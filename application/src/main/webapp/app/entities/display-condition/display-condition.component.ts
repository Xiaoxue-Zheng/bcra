...
export class DisplayConditionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.displayConditionService
      ...
      .subscribe(
        (res: HttpResponse<IDisplayCondition[]>) => this.paginateDisplayConditions(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/display-condition'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateDisplayConditions(data: IDisplayCondition[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.displayConditions = data;
  }
