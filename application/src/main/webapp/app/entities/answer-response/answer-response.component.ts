...
export class AnswerResponseComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.answerResponseService
      ...
      .subscribe(
        (res: HttpResponse<IAnswerResponse[]>) => this.paginateAnswerResponses(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/answer-response'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateAnswerResponses(data: IAnswerResponse[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.answerResponses = data;
  }
