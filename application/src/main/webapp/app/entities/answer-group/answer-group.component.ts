...
export class AnswerGroupComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.answerGroupService
      ...
      .subscribe(
        (res: HttpResponse<IAnswerGroup[]>) => this.paginateAnswerGroups(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/answer-group'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateAnswerGroups(data: IAnswerGroup[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.answerGroups = data;
  }
